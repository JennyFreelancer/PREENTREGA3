...
const cargarEgresos = ()=>{
    ...
    if (filter !== '') {
        results = results.filter((egreso) => {
            return egreso.descripcion.toLowerCase().includes(filter.toLowerCase())
        })
    }
...
var filter = ''
 ...
 const filterChange = () => {
     const value = document.getElementById('filter-ingresos').value
                   document.getElementById('filter-egresos').value
     filter = value
     cargarIngresos()
     cargarEgresos()
